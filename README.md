...
- **Mirrored codebase**: Android (Kotlin/Compose) and iOS (Swift/SwiftUI) modules map 1:1 — `Merger/`, `Editor/Engine/`, `Util/` — with identical algorithm parameters. link parsing sits behind a `LinkParser` protocol + registry on both (`linkparse/` on Android, `LinkParse/` on iOS)
